...
/**
 * InCore Function: elem_exp
 * Single-core tile computation function.
 * Called by PTO Runtime as a scheduled task.
 * NOT a kernel entry - use elem_exp_kernel_wrapper() to launch as kernel.
 */
class elem_expInCore {
public:
    // Single-core constructor - no block coordination
    __aicore__ inline elem_expInCore() {}

    // Initialize with global memory pointers
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output) {
        inputGm.SetGlobalBuffer((__gm__ float*)input);
        outputGm.SetGlobalBuffer((__gm__ float*)output);
        pipe.InitBuffer(inQueueX, 1, 512);
        pipe.InitBuffer(outQueueY, 1, 512);
    }
 ...
 */
__aicore__ inline void elem_exp(
 ...
 */
extern "C" __global__ __aicore__ void elem_exp_kernel(GM_ADDR input, GM_ADDR output) {
    elem_expInCore op;
