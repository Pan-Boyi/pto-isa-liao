...
/**
 * InCore Function: rowexpandsub
 * Single-core tile computation function.
 * Called by PTO Runtime as a scheduled task.
 * NOT a kernel entry - use rowexpandsub_kernel_wrapper() to launch as kernel.
 */
class rowexpandsubInCore {
public:
    // Single-core constructor - no block coordination
    __aicore__ inline rowexpandsubInCore() {}

    // Initialize with global memory pointers
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR output) {
        inputGm.SetGlobalBuffer((__gm__ float*)input);
        outputGm.SetGlobalBuffer((__gm__ float*)output);
        pipe.InitBuffer(inQueueX, 1, 544);
        pipe.InitBuffer(outQueueY, 1, 544);
    }
 ...
 */
__aicore__ inline void rowexpandsub(
 ...
 */
extern "C" __global__ __aicore__ void rowexpandsub_kernel(GM_ADDR input, GM_ADDR output) {
    rowexpandsubInCore op;
