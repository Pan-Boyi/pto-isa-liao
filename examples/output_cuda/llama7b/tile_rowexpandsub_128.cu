...
__global__ void tile_rowexpandsub_128_kernel(float* input_x, float* input_row, float* output) {
...
void tile_rowexpandsub_128(float* input_x, float* input_row, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_rowexpandsub_128_kernel<<<grid, block>>>(input_x, input_row, output);
