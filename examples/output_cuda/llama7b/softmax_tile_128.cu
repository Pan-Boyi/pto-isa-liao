...
//   --------------------------------------------------------------------------------
//   exp_x                128x128    f32     65536   [  3,   5]           <- x
//   result               128x128    f32     65536   [  5,   6]           <- x_shifted
//   row_max              128x1      f32       512   [  1,   2]           -
//   row_sum              128x1      f32       512   [  4,   5]           <- row_max
//   x                    128x128    f32     65536   [  0,   2]           -
//   x_shifted            128x128    f32     65536   [  2,   3]           -
//
// BUFFER REUSE MAP:
...
__device__ float row_max[128][1];
__device__ float x_shifted[128][128];
__device__ float exp_x[128][128];
__device__ float row_sum[128][1];
__device__ float result[128][128];

__global__ void softmax_tile_128_kernel(float* input, float* output) {
...
void softmax_tile_128(float* input, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    softmax_tile_128_kernel<<<grid, block>>>(input, output);
