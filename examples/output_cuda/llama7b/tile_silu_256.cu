...
//   --------------------------------------------------------------------------------
//   exp_neg_x            256x128    f32    131072   [  2,   3]           -
//   neg_x                256x128    f32    131072   [  1,   2]           -
//   one_plus_exp         256x128    f32    131072   [  3,   4]           <- neg_x
//   result               256x128    f32    131072   [  5,   6]           <- one_plus_exp
//   sigmoid              256x128    f32    131072   [  4,   5]           <- exp_neg_x
//   x                    256x128    f32    131072   [  0,   5]           -
//
// BUFFER REUSE MAP:
...
__device__ float neg_x[256][128];
__device__ float exp_neg_x[256][128];
__device__ float one_plus_exp[256][128];
__device__ float sigmoid[256][128];
__device__ float result[256][128];

__global__ void tile_silu_256_kernel(float* input, float* output) {
...
void tile_silu_256(float* input, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_silu_256_kernel<<<grid, block>>>(input, output);
