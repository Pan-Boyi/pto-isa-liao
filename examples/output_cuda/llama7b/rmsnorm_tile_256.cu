...
//   Total capacity (w/ reuse): 395,264 bytes (386.0 KB)
//   Reuse savings:            263,168 bytes (40.0%)
//
// TILE DETAILS:
//   Name                 Shape      Type   Bytes    Liveness [write,read]   Reuse
//   --------------------------------------------------------------------------------
//   gamma                256x128    f32    131072   [  1,  10]           -
//   result               256x128    f32    131072   [ 10,  11]           <- x
//   row_mean             256x1      f32      1024   [  5,   8]           -
//   row_rsqrt            256x1      f32      1024   [  8,   9]           <- row_sum
//   row_sum              256x1      f32      1024   [  3,   5]           -
//   x                    256x128    f32    131072   [  0,   9]           -
//   x_norm               256x128    f32    131072   [  9,  10]           <- x_sq
//   x_sq                 256x128    f32    131072   [  2,   3]           -
//
// BUFFER REUSE MAP:
...
__device__ float x_sq[256][128];
__device__ float row_sum[256][1];
__device__ float row_mean[256][1];
__device__ float row_rsqrt[256][1];
__device__ float x_norm[256][128];
__device__ float gamma[256][128];
__device__ float result[256][128];

__global__ void rmsnorm_tile_256_kernel(float* input, float* weights, float* output) {
...
void rmsnorm_tile_256(float* input, float* weights, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    rmsnorm_tile_256_kernel<<<grid, block>>>(input, weights, output, eps, inv_cols);
