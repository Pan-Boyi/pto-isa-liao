...
//   Total capacity (w/ reuse): 458,752 bytes (448.0 KB)
...
//   q                    256x128    f32    131072   [  0,  -1]           -
//   scaled_scores        256x128    f32    131072   [  4,   5]           -
//   scores               256x128    f32    131072   [  2,   4]           -
//
// ======================================================================
...
__device__ float q[256][128];
__device__ float k_t[128][128];
__device__ float scores[256][128];
__device__ float scaled_scores[256][128];

__global__ void attention_score_tile_256_kernel(float* input_q, float* input_kt, float* output) {
...
void attention_score_tile_256(float* input_q, float* input_kt, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_score_tile_256_kernel<<<grid, block>>>(input_q, input_kt, output, scale);
