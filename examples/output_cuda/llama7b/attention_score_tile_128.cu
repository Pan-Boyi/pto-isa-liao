...
//   q                    128x128    f32     65536   [  0,  -1]           -
//   scaled_scores        128x128    f32     65536   [  4,   5]           -
//   scores               128x128    f32     65536   [  2,   4]           -
//
// ======================================================================
...
__device__ float q[128][128];
__device__ float k_t[128][128];
__device__ float scores[128][128];
__device__ float scaled_scores[128][128];

__global__ void attention_score_tile_128_kernel(float* input_q, float* input_kt, float* output) {
    int _row = threadIdx.y + blockIdx.y * blockDim.y;
    int _col = threadIdx.x + blockIdx.x * blockDim.x;

    // Loop fusion: 2 loop overheads saved

    // FUSED (2 ops): q=TLOAD(...); k_t=TLOAD(...)
...
void attention_score_tile_128(float* input_q, float* input_kt, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_score_tile_128_kernel<<<grid, block>>>(input_q, input_kt, output, scale);
