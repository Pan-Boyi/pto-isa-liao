...
__global__ void tile_add_128_kernel(float* input_a, float* input_b, float* output) {
...
void tile_add_128(float* input_a, float* input_b, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_add_128_kernel<<<grid, block>>>(input_a, input_b, output);
