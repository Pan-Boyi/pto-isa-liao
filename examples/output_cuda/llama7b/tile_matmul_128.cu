...
__device__ float c[128][128];

__global__ void tile_matmul_128_kernel(float* input_a, float* input_b, float* output) {
...
void tile_matmul_128(float* input_a, float* input_b, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_matmul_128_kernel<<<grid, block>>>(input_a, input_b, output);
