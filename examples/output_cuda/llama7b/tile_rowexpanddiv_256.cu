...
__global__ void tile_rowexpanddiv_256_kernel(float* input_x, float* input_row, float* output) {
...
void tile_rowexpanddiv_256(float* input_x, float* input_row, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    tile_rowexpanddiv_256_kernel<<<grid, block>>>(input_x, input_row, output);
