...
//   --------------------------------------------------------------------------------
//   cos_pos              256x128    f32    131072   [  1,   3]           -
//   result               256x128    f32    131072   [  5,   6]           <- x
//   sin_pos              256x128    f32    131072   [  2,   4]           -
//   x                    256x128    f32    131072   [  0,   4]           -
//   x_cos                256x128    f32    131072   [  3,   5]           -
//   x_sin                256x128    f32    131072   [  4,   5]           <- cos_pos
//
// BUFFER REUSE MAP:
...
__device__ float cos_pos[256][128];
__device__ float sin_pos[256][128];
__device__ float x_cos[256][128];
__device__ float x_sin[256][128];
__device__ float result[256][128];

__global__ void rope_tile_256_kernel(float* input, float* cos_cache, float* sin_cache, float* output) {
...
void rope_tile_256(float* input, float* cos_cache, float* sin_cache, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    rope_tile_256_kernel<<<grid, block>>>(input, cos_cache, sin_cache, output);
