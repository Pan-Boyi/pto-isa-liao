...
//   weights              256x128    f32    131072   [  0,  -1]           -
//
// ======================================================================
...
__device__ float weights[256][128];
__device__ float v[128][128];
__device__ float result[256][128];

__global__ void attention_output_tile_256_kernel(float* input_weights, float* input_v, float* output) {
...
void attention_output_tile_256(float* input_weights, float* input_v, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    attention_output_tile_256_kernel<<<grid, block>>>(input_weights, input_v, output);
