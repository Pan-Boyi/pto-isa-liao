...
//   --------------------------------------------------------------------------------
//   residual             256x128    f32    131072   [  1,   2]           -
...
__device__ float residual[256][128];
__device__ float result[256][128];

__global__ void residual_add_tile_256_kernel(float* input, float* input_residual, float* output) {
...
void residual_add_tile_256(float* input, float* input_residual, float* output) {
    dim3 block(8, 8);
    dim3 grid(1, 1);
    residual_add_tile_256_kernel<<<grid, block>>>(input, input_residual, output);
