...
extern "C" __global__ AICORE void sparse_attention_antiquant_demo_kernel(__gm__ float __in__ *q_mem, __gm__ float __in__ *k_mem, __gm__ float __in__ *v_mem, __gm__ float __out__ *out_mem) {
    ...
    TASSIGN(row_max, 0x280);
    TASSIGN(shifted, 0x300);
    TASSIGN(exp_scores, 0x380);
    TASSIGN(sum_exp, 0x400);
    TASSIGN(probs, 0x480);
    TASSIGN(out, 0x500);
    TASSIGN(pto_tmp_0, 0x580);
    TASSIGN(pto_tmp_1, 0x600);
    TASSIGN(pto_tmp_2, 0x680);
    ...
    TROWMAX(row_max, scores, pto_tmp_1);
    TROWEXPANDSUB(shifted, scores, row_max);
    ...
    Global_out_mem g_out_mem_11(out_mem + (0) * 4 + (0));
    TSTORE(g_out_mem_11, out);
}
...
extern "C" const char* pto_program_name() { return "sparse_attention_antiquant_demo"; }
...
extern "C" void pto_launch(void **args, aclrtStream stream) {
    sparse_attention_antiquant_demo_kernel<<<1, nullptr, stream>>>((float*)args[0], (float*)args[1], (float*)args[2], (float*)args[3]);
}
