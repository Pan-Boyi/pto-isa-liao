...
static const int sparse_attention_antiquant_demo_is_cube = 0;

// Tile buffers (for scalar simulation)
static float _tile_sparse_attention_antiquant_demo_q[16];
static float _tile_sparse_attention_antiquant_demo_k[16];
static float _tile_sparse_attention_antiquant_demo_v[16];
static float _tile_sparse_attention_antiquant_demo_k_t[16];
static float _tile_sparse_attention_antiquant_demo_scores[16];
static float _tile_sparse_attention_antiquant_demo_row_max[4];
static float _tile_sparse_attention_antiquant_demo_shifted[16];
static float _tile_sparse_attention_antiquant_demo_exp_scores[16];
static float _tile_sparse_attention_antiquant_demo_sum_exp[4];
static float _tile_sparse_attention_antiquant_demo_probs[16];
static float _tile_sparse_attention_antiquant_demo_out[16];

int64_t sparse_attention_antiquant_demo_sim(float* q_mem, float* k_mem, float* v_mem, float* out_mem) {
    ...
    _cycle_count += 4 * CYCLE_REDUCTION_PER_ROW;  // TROWMAX scores
...
void sparse_attention_antiquant_demo(void** args, int32_t num_args) {
...
int64_t sparse_attention_antiquant_demo_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* q_mem = (float*)args[0];
    float* k_mem = (float*)args[1];
    float* v_mem = (float*)args[2];
    float* out_mem = (float*)args[3];
    return sparse_attention_antiquant_demo_sim(q_mem, k_mem, v_mem, out_mem);
}

// Get is_cube flag for this function
int sparse_attention_antiquant_demo_get_is_cube(void) {
    return sparse_attention_antiquant_demo_is_cube;
}