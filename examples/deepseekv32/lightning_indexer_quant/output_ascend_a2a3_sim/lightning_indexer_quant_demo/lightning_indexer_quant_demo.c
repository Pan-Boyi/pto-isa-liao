...
static const int lightning_indexer_quant_demo_is_cube = 0;

// Tile buffers (for scalar simulation)
static float _tile_lightning_indexer_quant_demo_q[16];
static float _tile_lightning_indexer_quant_demo_k[16];
static float _tile_lightning_indexer_quant_demo_k_t[16];
static float _tile_lightning_indexer_quant_demo_scores[16];

int64_t lightning_indexer_quant_demo_sim(float* q_mem, float* k_mem, float* scores_mem) {
    ...
    _cycle_count += (int64_t)(16 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE scores

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void lightning_indexer_quant_demo(void** args, int32_t num_args) {
...
int64_t lightning_indexer_quant_demo_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* q_mem = (float*)args[0];
    float* k_mem = (float*)args[1];
    float* scores_mem = (float*)args[2];
    return lightning_indexer_quant_demo_sim(q_mem, k_mem, scores_mem);
}

// Get is_cube flag for this function
int lightning_indexer_quant_demo_get_is_cube(void) {
    return lightning_indexer_quant_demo_is_cube;
}