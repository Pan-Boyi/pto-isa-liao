...
int64_t rowsum_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(64 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 8 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x
...
void rowsum(void** args, int32_t num_args) {
...
int64_t rowsum_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return rowsum_sim(input, output);
}