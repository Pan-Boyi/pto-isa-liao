...
static float _tile_x[64];
static float _tile_row_vals[8];
static float _tile_result[64];

int64_t rowexpandsub_sim(float* input_x, float* input_row, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(64 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(8 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD row_vals
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDSUB result
    _cycle_count += (int64_t)(64 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE result

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void rowexpandsub(void** args, int32_t num_args) {
...
int64_t rowexpandsub_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return rowexpandsub_sim(input_x, input_row, output);
}