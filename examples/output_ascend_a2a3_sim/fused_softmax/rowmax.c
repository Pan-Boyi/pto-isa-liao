...
static float _tile_result[8];

int64_t rowmax_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(64 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 8 * CYCLE_REDUCTION_PER_ROW;  // TROWMAX x
    _cycle_count += (int64_t)(8 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE result

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void rowmax(void** args, int32_t num_args) {
...
int64_t rowmax_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return rowmax_sim(input, output);
}