...
static float _tile_rope_tile_x[4096];
static float _tile_rope_tile_cos_pos[4096];
static float _tile_rope_tile_sin_pos[4096];
static float _tile_rope_tile_x_cos[4096];
static float _tile_rope_tile_x_sin[4096];
static float _tile_rope_tile_result[4096];

int64_t rope_tile_sim(float* input, float* cos_cache, float* sin_cache, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD cos_pos
    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD sin_pos
    _cycle_count += CYCLE_VECTOR_OP;  // TMUL x_cos
    _cycle_count += CYCLE_VECTOR_OP;  // TMUL x_sin
...
void rope_tile(void** args, int32_t num_args) {
...
int64_t rope_tile_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* cos_cache = (float*)args[1];
    float* sin_cache = (float*)args[2];
    float* output = (float*)args[3];
    return rope_tile_sim(input, cos_cache, sin_cache, output);
}