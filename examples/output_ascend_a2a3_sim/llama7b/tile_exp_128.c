...
static float _tile_tile_exp_128_x[16384];
static float _tile_tile_exp_128_result[16384];

int64_t tile_exp_128_sim(float* input, float* output) {
...
void tile_exp_128(void** args, int32_t num_args) {
...
int64_t tile_exp_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_exp_128_sim(input, output);
}