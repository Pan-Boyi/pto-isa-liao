...
static float _tile_attention_score_tile_256_q[32768];
static float _tile_attention_score_tile_256_k_t[16384];
static float _tile_attention_score_tile_256_scores[32768];
static float _tile_attention_score_tile_256_scaled_scores[32768];

int64_t attention_score_tile_256_sim(float* input_q, float* input_kt, float* output, float scale) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD q
    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD k_t
    _cycle_count += (int64_t)(4194304 * CYCLE_MATMUL_PER_MAC);  // TMATMUL
    scale = 0.08838834764831843;
    _cycle_count += CYCLE_VECTOR_OP;  // TMULS scaled_scores
    _cycle_count += (int64_t)(32768 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE scaled_scores

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void attention_score_tile_256(void** args, int32_t num_args) {
...
int64_t attention_score_tile_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_q = (float*)args[0];
    float* input_kt = (float*)args[1];
    float* output = (float*)args[2];
    float scale = *(float*)args[3];
    return attention_score_tile_256_sim(input_q, input_kt, output, scale);
}