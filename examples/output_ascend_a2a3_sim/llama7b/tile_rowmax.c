...
static float _tile_tile_rowmax_x[4096];
static float _tile_tile_rowmax_result[32];

int64_t tile_rowmax_sim(float* input, float* output) {
    ...
    _cycle_count += (int64_t)(32 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE result

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void tile_rowmax(void** args, int32_t num_args) {
...
int64_t tile_rowmax_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowmax_sim(input, output);
}