...
static float _tile_swiglu_tile_64_gate[8192];
static float _tile_swiglu_tile_64_up[8192];
static float _tile_swiglu_tile_64_neg_gate[8192];
static float _tile_swiglu_tile_64_exp_neg_gate[8192];
static float _tile_swiglu_tile_64_one_plus_exp[8192];
static float _tile_swiglu_tile_64_sigmoid_gate[8192];
static float _tile_swiglu_tile_64_gate_silu[8192];
static float _tile_swiglu_tile_64_result[8192];

int64_t swiglu_tile_64_sim(float* input_gate, float* input_up, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD gate
    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD up
...
void swiglu_tile_64(void** args, int32_t num_args) {
...
int64_t swiglu_tile_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_gate = (float*)args[0];
    float* input_up = (float*)args[1];
    float* output = (float*)args[2];
    return swiglu_tile_64_sim(input_gate, input_up, output);
}