...
static float _tile_tile_muls_64_a[8192];
static float _tile_tile_muls_64_result[8192];

int64_t tile_muls_64_sim(float* input, float* output, float scale) {
...
void tile_muls_64(void** args, int32_t num_args) {
...
int64_t tile_muls_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    float scale = *(float*)args[2];
    return tile_muls_64_sim(input, output, scale);
}