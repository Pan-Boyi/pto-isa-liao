...
static float _tile_tile_add_128_a[16384];
static float _tile_tile_add_128_b[16384];
static float _tile_tile_add_128_result[16384];

int64_t tile_add_128_sim(float* input_a, float* input_b, float* output) {
...
void tile_add_128(void** args, int32_t num_args) {
...
int64_t tile_add_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_add_128_sim(input_a, input_b, output);
}