...
static float _tile_softmax_tile_64_x[8192];
static float _tile_softmax_tile_64_row_max[64];
static float _tile_softmax_tile_64_x_shifted[8192];
static float _tile_softmax_tile_64_exp_x[8192];
static float _tile_softmax_tile_64_row_sum[64];
static float _tile_softmax_tile_64_result[8192];

int64_t softmax_tile_64_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 64 * CYCLE_REDUCTION_PER_ROW;  // TROWMAX x
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDSUB x_shifted
    _cycle_count += CYCLE_VECTOR_OP;  // TEXP exp_x
    _cycle_count += 64 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM exp_x
...
void softmax_tile_64(void** args, int32_t num_args) {
...
int64_t softmax_tile_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return softmax_tile_64_sim(input, output);
}