...
static float _tile_attention_output_tile_weights[4096];
static float _tile_attention_output_tile_v[16384];
static float _tile_attention_output_tile_result[4096];

int64_t attention_output_tile_sim(float* input_weights, float* input_v, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD weights
...
void attention_output_tile(void** args, int32_t num_args) {
...
int64_t attention_output_tile_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_weights = (float*)args[0];
    float* input_v = (float*)args[1];
    float* output = (float*)args[2];
    return attention_output_tile_sim(input_weights, input_v, output);
}