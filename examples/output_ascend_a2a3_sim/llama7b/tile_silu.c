...
static float _tile_tile_silu_x[4096];
static float _tile_tile_silu_neg_x[4096];
static float _tile_tile_silu_exp_neg_x[4096];
static float _tile_tile_silu_one_plus_exp[4096];
static float _tile_tile_silu_sigmoid[4096];
static float _tile_tile_silu_result[4096];

int64_t tile_silu_sim(float* input, float* output) {
...
void tile_silu(void** args, int32_t num_args) {
...
int64_t tile_silu_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_silu_sim(input, output);
}