...
static float _tile_tile_rowexpanddiv_x[4096];
static float _tile_tile_rowexpanddiv_row_vals[32];
static float _tile_tile_rowexpanddiv_result[4096];

int64_t tile_rowexpanddiv_sim(float* input_x, float* input_row, float* output) {
...
void tile_rowexpanddiv(void** args, int32_t num_args) {
...
int64_t tile_rowexpanddiv_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpanddiv_sim(input_x, input_row, output);
}