...
static float _tile_tile_rowexpandsub_128_x[16384];
static float _tile_tile_rowexpandsub_128_row_vals[128];
static float _tile_tile_rowexpandsub_128_result[16384];

int64_t tile_rowexpandsub_128_sim(float* input_x, float* input_row, float* output) {
...
void tile_rowexpandsub_128(void** args, int32_t num_args) {
...
int64_t tile_rowexpandsub_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandsub_128_sim(input_x, input_row, output);
}