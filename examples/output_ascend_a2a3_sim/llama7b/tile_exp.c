...
static float _tile_tile_exp_x[4096];
static float _tile_tile_exp_result[4096];

int64_t tile_exp_sim(float* input, float* output) {
...
void tile_exp(void** args, int32_t num_args) {
...
int64_t tile_exp_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_exp_sim(input, output);
}