...
static float _tile_softmax_tile_x[4096];
static float _tile_softmax_tile_row_max[32];
static float _tile_softmax_tile_x_shifted[4096];
static float _tile_softmax_tile_exp_x[4096];
static float _tile_softmax_tile_row_sum[32];
static float _tile_softmax_tile_result[4096];

int64_t softmax_tile_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 32 * CYCLE_REDUCTION_PER_ROW;  // TROWMAX x
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDSUB x_shifted
    _cycle_count += CYCLE_VECTOR_OP;  // TEXP exp_x
    _cycle_count += 32 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM exp_x
...
void softmax_tile(void** args, int32_t num_args) {
...
int64_t softmax_tile_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return softmax_tile_sim(input, output);
}