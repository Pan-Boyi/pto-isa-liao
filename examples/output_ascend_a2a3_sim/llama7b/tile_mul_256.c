...
static float _tile_tile_mul_256_a[32768];
static float _tile_tile_mul_256_b[32768];
static float _tile_tile_mul_256_result[32768];

int64_t tile_mul_256_sim(float* input_a, float* input_b, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD a
    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD b
...
void tile_mul_256(void** args, int32_t num_args) {
...
int64_t tile_mul_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_mul_256_sim(input_a, input_b, output);
}