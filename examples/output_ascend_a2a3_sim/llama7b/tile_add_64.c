...
static float _tile_tile_add_64_a[8192];
static float _tile_tile_add_64_b[8192];
static float _tile_tile_add_64_result[8192];

int64_t tile_add_64_sim(float* input_a, float* input_b, float* output) {
...
void tile_add_64(void** args, int32_t num_args) {
...
int64_t tile_add_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_add_64_sim(input_a, input_b, output);
}