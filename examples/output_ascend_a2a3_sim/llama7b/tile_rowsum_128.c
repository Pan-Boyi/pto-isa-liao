...
static float _tile_tile_rowsum_128_x[16384];
static float _tile_tile_rowsum_128_result[128];

int64_t tile_rowsum_128_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 128 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x
...
void tile_rowsum_128(void** args, int32_t num_args) {
...
int64_t tile_rowsum_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowsum_128_sim(input, output);
}