// PTO Program: tile_rowexpandsub_256
...
static float _tile_tile_rowexpandsub_256_x[32768];
static float _tile_tile_rowexpandsub_256_row_vals[256];
static float _tile_tile_rowexpandsub_256_result[32768];

int64_t tile_rowexpandsub_256_sim(float* input_x, float* input_row, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(256 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD row_vals
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDSUB result
...
void tile_rowexpandsub_256(void** args, int32_t num_args) {
...
int64_t tile_rowexpandsub_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandsub_256_sim(input_x, input_row, output);
}