...
static float _tile_flash_attn_normalize_o_block[8192];
static float _tile_flash_attn_normalize_l_vec[64];
static float _tile_flash_attn_normalize_o_final[8192];

int64_t flash_attn_normalize_sim(float* input_o, float* input_l, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD o_block
    _cycle_count += (int64_t)(64 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD l_vec
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDDIV o_final
    _cycle_count += (int64_t)(8192 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE o_final

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void flash_attn_normalize(void** args, int32_t num_args) {
...
int64_t flash_attn_normalize_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_o = (float*)args[0];
    float* input_l = (float*)args[1];
    float* output = (float*)args[2];
    return flash_attn_normalize_sim(input_o, input_l, output);
}