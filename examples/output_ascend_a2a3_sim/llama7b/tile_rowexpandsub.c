...
static float _tile_tile_rowexpandsub_x[4096];
static float _tile_tile_rowexpandsub_row_vals[32];
static float _tile_tile_rowexpandsub_result[4096];

int64_t tile_rowexpandsub_sim(float* input_x, float* input_row, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(32 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD row_vals
...
void tile_rowexpandsub(void** args, int32_t num_args) {
...
int64_t tile_rowexpandsub_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandsub_sim(input_x, input_row, output);
}