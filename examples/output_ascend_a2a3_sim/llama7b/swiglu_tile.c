...
static float _tile_swiglu_tile_gate[4096];
static float _tile_swiglu_tile_up[4096];
static float _tile_swiglu_tile_neg_gate[4096];
static float _tile_swiglu_tile_exp_neg_gate[4096];
static float _tile_swiglu_tile_one_plus_exp[4096];
static float _tile_swiglu_tile_sigmoid_gate[4096];
static float _tile_swiglu_tile_gate_silu[4096];
static float _tile_swiglu_tile_result[4096];

int64_t swiglu_tile_sim(float* input_gate, float* input_up, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD gate
    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD up
...
void swiglu_tile(void** args, int32_t num_args) {
...
int64_t swiglu_tile_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_gate = (float*)args[0];
    float* input_up = (float*)args[1];
    float* output = (float*)args[2];
    return swiglu_tile_sim(input_gate, input_up, output);
}