...
static float _tile_tile_exp_256_x[32768];
static float _tile_tile_exp_256_result[32768];

int64_t tile_exp_256_sim(float* input, float* output) {
...
void tile_exp_256(void** args, int32_t num_args) {
...
int64_t tile_exp_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_exp_256_sim(input, output);
}