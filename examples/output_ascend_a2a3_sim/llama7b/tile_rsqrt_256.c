...
static float _tile_tile_rsqrt_256_x[32768];
static float _tile_tile_rsqrt_256_result[32768];

int64_t tile_rsqrt_256_sim(float* input, float* output) {
...
void tile_rsqrt_256(void** args, int32_t num_args) {
...
int64_t tile_rsqrt_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rsqrt_256_sim(input, output);
}