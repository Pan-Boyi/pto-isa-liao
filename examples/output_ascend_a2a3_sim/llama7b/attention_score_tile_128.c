...
static float _tile_attention_score_tile_128_q[16384];
static float _tile_attention_score_tile_128_k_t[16384];
static float _tile_attention_score_tile_128_scores[16384];
static float _tile_attention_score_tile_128_scaled_scores[16384];

int64_t attention_score_tile_128_sim(float* input_q, float* input_kt, float* output, float scale) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD q
    ...
    _cycle_count += (int64_t)(16384 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE scaled_scores

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void attention_score_tile_128(void** args, int32_t num_args) {
...
int64_t attention_score_tile_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_q = (float*)args[0];
    float* input_kt = (float*)args[1];
    float* output = (float*)args[2];
    float scale = *(float*)args[3];
    return attention_score_tile_128_sim(input_q, input_kt, output, scale);
}