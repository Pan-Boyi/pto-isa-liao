...
static float _tile_residual_add_tile_128_x[16384];
static float _tile_residual_add_tile_128_residual[16384];
static float _tile_residual_add_tile_128_result[16384];

int64_t residual_add_tile_128_sim(float* input, float* input_residual, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD residual
...
void residual_add_tile_128(void** args, int32_t num_args) {
...
int64_t residual_add_tile_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* input_residual = (float*)args[1];
    float* output = (float*)args[2];
    return residual_add_tile_128_sim(input, input_residual, output);
}