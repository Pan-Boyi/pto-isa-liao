...
static float _tile_tile_rowmax_128_x[16384];
static float _tile_tile_rowmax_128_result[128];

int64_t tile_rowmax_128_sim(float* input, float* output) {
    ...
    _cycle_count += (int64_t)(128 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE result

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void tile_rowmax_128(void** args, int32_t num_args) {
...
int64_t tile_rowmax_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowmax_128_sim(input, output);
}