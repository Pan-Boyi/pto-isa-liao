...
static float _tile_tile_mul_a[4096];
static float _tile_tile_mul_b[4096];
static float _tile_tile_mul_result[4096];

int64_t tile_mul_sim(float* input_a, float* input_b, float* output) {
...
void tile_mul(void** args, int32_t num_args) {
...
int64_t tile_mul_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_mul_sim(input_a, input_b, output);
}