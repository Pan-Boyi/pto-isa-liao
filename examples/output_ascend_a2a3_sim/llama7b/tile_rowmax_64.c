...
static float _tile_tile_rowmax_64_x[8192];
static float _tile_tile_rowmax_64_result[64];

int64_t tile_rowmax_64_sim(float* input, float* output) {
...
void tile_rowmax_64(void** args, int32_t num_args) {
...
int64_t tile_rowmax_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowmax_64_sim(input, output);
}