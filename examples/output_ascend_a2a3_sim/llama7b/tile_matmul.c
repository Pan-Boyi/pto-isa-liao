...
static float _tile_tile_matmul_a[4096];
static float _tile_tile_matmul_b[16384];
static float _tile_tile_matmul_c[4096];

int64_t tile_matmul_sim(float* input_a, float* input_b, float* output) {
    ...
    _cycle_count += (int64_t)(4096 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE c

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void tile_matmul(void** args, int32_t num_args) {
...
int64_t tile_matmul_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_matmul_sim(input_a, input_b, output);
}