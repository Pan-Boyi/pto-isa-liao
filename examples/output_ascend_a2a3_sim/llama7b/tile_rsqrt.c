...
static float _tile_tile_rsqrt_x[4096];
static float _tile_tile_rsqrt_result[4096];

int64_t tile_rsqrt_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += CYCLE_VECTOR_OP;  // TRSQRT result
...
void tile_rsqrt(void** args, int32_t num_args) {
...
int64_t tile_rsqrt_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rsqrt_sim(input, output);
}