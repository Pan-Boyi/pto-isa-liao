...
static float _tile_tile_rowexpandmul_256_x[32768];
static float _tile_tile_rowexpandmul_256_row_vals[256];
static float _tile_tile_rowexpandmul_256_result[32768];

int64_t tile_rowexpandmul_256_sim(float* input_x, float* input_row, float* output) {
...
void tile_rowexpandmul_256(void** args, int32_t num_args) {
...
int64_t tile_rowexpandmul_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandmul_256_sim(input_x, input_row, output);
}