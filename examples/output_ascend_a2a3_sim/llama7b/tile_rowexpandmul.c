...
static float _tile_tile_rowexpandmul_x[4096];
static float _tile_tile_rowexpandmul_row_vals[32];
static float _tile_tile_rowexpandmul_result[4096];

int64_t tile_rowexpandmul_sim(float* input_x, float* input_row, float* output) {
...
void tile_rowexpandmul(void** args, int32_t num_args) {
...
int64_t tile_rowexpandmul_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandmul_sim(input_x, input_row, output);
}