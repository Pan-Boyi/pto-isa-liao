...
static float _tile_tile_rowexpandsub_64_x[8192];
static float _tile_tile_rowexpandsub_64_row_vals[64];
static float _tile_tile_rowexpandsub_64_result[8192];

int64_t tile_rowexpandsub_64_sim(float* input_x, float* input_row, float* output) {
...
void tile_rowexpandsub_64(void** args, int32_t num_args) {
...
int64_t tile_rowexpandsub_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandsub_64_sim(input_x, input_row, output);
}