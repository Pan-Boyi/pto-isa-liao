...
static float _tile_tile_muls_a[4096];
static float _tile_tile_muls_result[4096];

int64_t tile_muls_sim(float* input, float* output, float scale) {
...
void tile_muls(void** args, int32_t num_args) {
...
int64_t tile_muls_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    float scale = *(float*)args[2];
    return tile_muls_sim(input, output, scale);
}