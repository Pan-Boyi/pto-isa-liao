...
static float _tile_rmsnorm_tile_x[4096];
static float _tile_rmsnorm_tile_x_sq[4096];
static float _tile_rmsnorm_tile_row_sum[32];
static float _tile_rmsnorm_tile_row_mean[32];
static float _tile_rmsnorm_tile_row_rsqrt[32];
static float _tile_rmsnorm_tile_x_norm[4096];
static float _tile_rmsnorm_tile_gamma[4096];
static float _tile_rmsnorm_tile_result[4096];

int64_t rmsnorm_tile_sim(float* input, float* weights, float* output, float eps, float inv_cols) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD gamma
    _cycle_count += CYCLE_VECTOR_OP;  // TMUL x_sq
    _cycle_count += 32 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x_sq
    inv_cols = 0.0078125;
...
void rmsnorm_tile(void** args, int32_t num_args) {
...
int64_t rmsnorm_tile_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    ...
    return rmsnorm_tile_sim(input, weights, output, eps, inv_cols);
}