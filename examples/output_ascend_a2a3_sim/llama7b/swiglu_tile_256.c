...
static float _tile_swiglu_tile_256_gate[32768];
static float _tile_swiglu_tile_256_up[32768];
static float _tile_swiglu_tile_256_neg_gate[32768];
static float _tile_swiglu_tile_256_exp_neg_gate[32768];
static float _tile_swiglu_tile_256_one_plus_exp[32768];
static float _tile_swiglu_tile_256_sigmoid_gate[32768];
static float _tile_swiglu_tile_256_gate_silu[32768];
static float _tile_swiglu_tile_256_result[32768];

int64_t swiglu_tile_256_sim(float* input_gate, float* input_up, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD gate
    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD up
...
void swiglu_tile_256(void** args, int32_t num_args) {
...
int64_t swiglu_tile_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_gate = (float*)args[0];
    float* input_up = (float*)args[1];
    float* output = (float*)args[2];
    return swiglu_tile_256_sim(input_gate, input_up, output);
}