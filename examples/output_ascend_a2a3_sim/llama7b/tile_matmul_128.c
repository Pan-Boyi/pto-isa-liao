...
static float _tile_tile_matmul_128_a[16384];
static float _tile_tile_matmul_128_b[16384];
static float _tile_tile_matmul_128_c[16384];

int64_t tile_matmul_128_sim(float* input_a, float* input_b, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD a
    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD b
    _cycle_count += (int64_t)(2097152 * CYCLE_MATMUL_PER_MAC);  // TMATMUL
    _cycle_count += (int64_t)(16384 * CYCLE_TSTORE_PER_ELEMENT);  // TSTORE c

    return _cycle_count;
}

// PTO Runtime compatible wrapper
void tile_matmul_128(void** args, int32_t num_args) {
...
int64_t tile_matmul_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_a = (float*)args[0];
    float* input_b = (float*)args[1];
    float* output = (float*)args[2];
    return tile_matmul_128_sim(input_a, input_b, output);
}