...
static float _tile_tile_rowsum_x[4096];
static float _tile_tile_rowsum_result[32];

int64_t tile_rowsum_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(4096 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 32 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x
...
void tile_rowsum(void** args, int32_t num_args) {
...
int64_t tile_rowsum_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowsum_sim(input, output);
}