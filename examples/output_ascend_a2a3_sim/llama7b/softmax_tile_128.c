...
static float _tile_softmax_tile_128_x[16384];
static float _tile_softmax_tile_128_row_max[128];
static float _tile_softmax_tile_128_x_shifted[16384];
static float _tile_softmax_tile_128_exp_x[16384];
static float _tile_softmax_tile_128_row_sum[128];
static float _tile_softmax_tile_128_result[16384];

int64_t softmax_tile_128_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 128 * CYCLE_REDUCTION_PER_ROW;  // TROWMAX x
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDSUB x_shifted
    _cycle_count += CYCLE_VECTOR_OP;  // TEXP exp_x
    _cycle_count += 128 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM exp_x
...
void softmax_tile_128(void** args, int32_t num_args) {
...
int64_t softmax_tile_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return softmax_tile_128_sim(input, output);
}