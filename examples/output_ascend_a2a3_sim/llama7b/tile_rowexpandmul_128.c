// PTO Program: tile_rowexpandmul_128
...
static float _tile_tile_rowexpandmul_128_x[16384];
static float _tile_tile_rowexpandmul_128_row_vals[128];
static float _tile_tile_rowexpandmul_128_result[16384];

int64_t tile_rowexpandmul_128_sim(float* input_x, float* input_row, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(128 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD row_vals
    _cycle_count += CYCLE_VECTOR_OP;  // TROWEXPANDMUL result
...
void tile_rowexpandmul_128(void** args, int32_t num_args) {
...
int64_t tile_rowexpandmul_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input_x = (float*)args[0];
    float* input_row = (float*)args[1];
    float* output = (float*)args[2];
    return tile_rowexpandmul_128_sim(input_x, input_row, output);
}