...
static float _tile_linear_tile_256_x[32768];
static float _tile_linear_tile_256_w[16384];
static float _tile_linear_tile_256_result[32768];

int64_t linear_tile_256_sim(float* input, float* weight, float* output) {
...
void linear_tile_256(void** args, int32_t num_args) {
...
int64_t linear_tile_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* weight = (float*)args[1];
    float* output = (float*)args[2];
    return linear_tile_256_sim(input, weight, output);
}