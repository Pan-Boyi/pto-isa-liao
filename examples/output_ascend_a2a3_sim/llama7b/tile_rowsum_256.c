...
static float _tile_tile_rowsum_256_x[32768];
static float _tile_tile_rowsum_256_result[256];

int64_t tile_rowsum_256_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(32768 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 256 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x
...
void tile_rowsum_256(void** args, int32_t num_args) {
...
int64_t tile_rowsum_256_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowsum_256_sim(input, output);
}