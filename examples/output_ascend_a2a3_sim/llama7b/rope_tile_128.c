...
static float _tile_rope_tile_128_x[16384];
static float _tile_rope_tile_128_cos_pos[16384];
static float _tile_rope_tile_128_sin_pos[16384];
static float _tile_rope_tile_128_x_cos[16384];
static float _tile_rope_tile_128_x_sin[16384];
static float _tile_rope_tile_128_result[16384];

int64_t rope_tile_128_sim(float* input, float* cos_cache, float* sin_cache, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD cos_pos
    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD sin_pos
...
void rope_tile_128(void** args, int32_t num_args) {
...
int64_t rope_tile_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* cos_cache = (float*)args[1];
    float* sin_cache = (float*)args[2];
    float* output = (float*)args[3];
    return rope_tile_128_sim(input, cos_cache, sin_cache, output);
}