...
static float _tile_tile_muls_128_a[16384];
static float _tile_tile_muls_128_result[16384];

int64_t tile_muls_128_sim(float* input, float* output, float scale) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(16384 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD a
    _cycle_count += CYCLE_VECTOR_OP;  // TMULS result
...
void tile_muls_128(void** args, int32_t num_args) {
...
int64_t tile_muls_128_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    float scale = *(float*)args[2];
    return tile_muls_128_sim(input, output, scale);
}