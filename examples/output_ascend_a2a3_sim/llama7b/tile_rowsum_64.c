...
static float _tile_tile_rowsum_64_x[8192];
static float _tile_tile_rowsum_64_result[64];

int64_t tile_rowsum_64_sim(float* input, float* output) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += 64 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x
...
void tile_rowsum_64(void** args, int32_t num_args) {
...
int64_t tile_rowsum_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    float* input = (float*)args[0];
    float* output = (float*)args[1];
    return tile_rowsum_64_sim(input, output);
}