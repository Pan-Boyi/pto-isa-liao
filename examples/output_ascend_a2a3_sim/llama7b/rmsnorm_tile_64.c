...
static float _tile_rmsnorm_tile_64_x[8192];
static float _tile_rmsnorm_tile_64_x_sq[8192];
static float _tile_rmsnorm_tile_64_row_sum[64];
static float _tile_rmsnorm_tile_64_row_mean[64];
static float _tile_rmsnorm_tile_64_row_rsqrt[64];
static float _tile_rmsnorm_tile_64_x_norm[8192];
static float _tile_rmsnorm_tile_64_gamma[8192];
static float _tile_rmsnorm_tile_64_result[8192];

int64_t rmsnorm_tile_64_sim(float* input, float* weights, float* output, float eps, float inv_cols) {
    int64_t _cycle_count = 0;


    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD x
    _cycle_count += (int64_t)(8192 * CYCLE_TLOAD_PER_ELEMENT);  // TLOAD gamma
    _cycle_count += CYCLE_VECTOR_OP;  // TMUL x_sq
    _cycle_count += 64 * CYCLE_REDUCTION_PER_ROW;  // TROWSUM x_sq
    inv_cols = 0.0078125;
...
void rmsnorm_tile_64(void** args, int32_t num_args) {
...
int64_t rmsnorm_tile_64_cycle_count(void** args, int32_t num_args) {
    (void)num_args;
    ...
    return rmsnorm_tile_64_sim(input, weights, output, eps, inv_cols);
}